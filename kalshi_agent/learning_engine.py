...
class LearningEngine:
    ...
    def log_trade(
        ...
        cur = self._conn.execute(
            ...
            INSERT INTO trades
                (timestamp, ticker, event_ticker, title, series_ticker, category,
                 side, action, count, entry_price, confidence, edge_score,
                 liquidity_score, volume_score, timing_score, momentum_score,
                 rationale, outcome)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending')
            """,
        ...
        logger.info(
            "Trade logged (id=%d): %s %s %s x%d @ %d¢ [conf=%.1f cat=%s]",
            trade_id, action, side, ticker, count, entry_price, confidence, category,
        )
        return trade_id
